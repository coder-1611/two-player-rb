...
(async () => {
  ...
  const now = Date.now();
  if (false) await fetch(FB + '/rooms/' + room + '/patDuty.json', { method: 'PUT',
  ...
  await B.screenshot({ path: SHOT + 'pat-resume-B2.png' }).catch(() => {});
  console.log('VERDICT: ' + (sawModal
    ? 'PAT modal appeared on resume'
    : 'NO MODAL — blank-field bug reproduced (see pat-resume-B2.png)'));
  await fetch(FB + '/rooms/' + room + '.json', { method: 'DELETE' }).catch(() => {});
  await browser.close();
