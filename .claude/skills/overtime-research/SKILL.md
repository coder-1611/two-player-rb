...
- **V117** — pin the engine in ONE never-ending OT period. The prior clock top-up ran only on the OFFENSE device, so the WAITING side's clock expired → the engine rolled the quarter (`_Wy++` → `@quarter_6`), looped through period-ends ("behaved like Q4 over and over"), or hit the clock-0 Wy>=5 freeze. Now BOTH the per-frame `_Ib1` wrapper and the 300ms coordinator, while `inOvertime`, clamp `_Wy` ≤ 5 and keep the clock ≥ 5:00 (top to 9:00) on BOTH devices. Headless-verified: forced `_Wy=6`/clock 0 → clamped back to `_Wy=5`/9:00. Diagnosed from the live Firebase room dump (`rooms/{code}` → `live`/`ot`/`config`).
- **V118** — ignore STALE `ot/p{period}` coin flips. Diagnosed from the live Firebase dump: a 6-hour-old `ot/p5` lingered in a reused room (match-start `ot` cleanup is skipped on reload/resume) and was re-applied on a FRESH match — forcing the receiver onto offense at Q1 with the 9:00 OT clock (live showed `quarter=1, min=9` for a 1-min game), which corrupted the whole game (showed "4th quarter", possessions looped back). The OT subscription now ignores + purges any entry whose `ts` is missing or older than 60 s. **When diagnosing OT, ALWAYS dump `rooms/{code}` via REST** (`curl <db>/rooms/{code}.json`) — `live`/`ot`/`config`/`outcomes` reveal the true cross-device state.
- **Still open / verify on 2 devices:** answering possession + multi-round in live play; the OT-start freeze; the post-turnover possession loop (was likely downstream of the stale-OT corruption — re-test after V118).
